...
#lbl_line_server.pack_forget() #nascosto
lbl_line_server.pack() #modificato per vedere qualcosa
...
lbl_opponent_name = tk.Label(top_left_frame, text="Opponent: " + "Player")
...
#write in a new label, the question
lbl_round = lab(round_frame, 'option_question')
create_button(round_frame, 'option_answer', 'option', len(option_dictionary['option_answer']))
...
btn_rock = tk.Button(button_frame, text="Rock", command=lambda : choice("rock"), state=tk.DISABLED)
btn_paper = tk.Button(button_frame, text="Paper", command=lambda : choice("paper"), state=tk.DISABLED)
btn_scissors = tk.Button(button_frame, text="Scissors", command=lambda : choice("scissors"), state=tk.DISABLED)
btn_rock.grid(row=0, column=0)
btn_paper.grid(row=0, column=1)
btn_scissors.grid(row=0, column=2)
button_frame.pack(side=tk.BOTTOM)

#function that take create a list of answer
#parameters: frame(the frame in which insert the buttons)
#list: the list of answers
def create_button(Frame, inner_dictionary, outer_dictionary, num_answers):
    while num_answers != 0:
        bt = tk.Button(Frame, text= data[inner_dictionary][num_answers][outer_dictionary])
        num_answers = num_answers -1

def create_question_label(Frame, dictionary):
    lab = tk.Label(Frame, text = option_dictionary[dictionary])
    return lab
